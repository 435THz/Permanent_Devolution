...
function SkillSelectMenu.runRelearnMenu(chara)
    ...
    local choose = function(move) ret = move end
    local refuse = function() _MENU:RemoveMenu() end
    local menu = SkillSelectMenu_Devolve:new(STRINGS:FormatKey("MENU_SKILL_RECALL"), chara, forgottenSkills, choose, refuse)
    UI:SetCustomMenu(menu.menu)
    UI:WaitForChoice()
    return ret
end

SkillSelectMenu_Devolve = Class("SkillSelectMenu_Devolve", SkillSelectMenu)
...
--- @param refuse_action function the function called when the player presses the cancel or menu button.
--- @param menu_width number the width of this window. Default is 152.
function SkillSelectMenu_Devolve:initialize(title, character, skill_list, confirm_action, refuse_action, menu_width)
    SkillSelectMenu.initialize(self, title, character, skill_list, confirm_action, refuse_action, menu_width)
end
...
--- @return table a list of ``RogueEssence.Menu.MenuElementChoice`` objects.
function SkillSelectMenu_Devolve:generate_options()
    local options = {}
    for i=1, #self.skillList, 1 do
        local skill = _DATA:GetSkill(self.skillList[i])
        local enabled = DEVOLUTION_MOD.CanLearnSkill(self.character, self.skillList[i])
        local color = "#FF0000"
        if enabled then color = "#00FF00" end
        local max_charges = skill.BaseCharges
        if self.character then max_charges = max_charges + self.character.ChargeBoost end

        local skill_name = "[color="..color.."]"..skill.Name:ToLocal().."[color]"
        local skill_charges = "[color="..color.."]"..max_charges.."/"..max_charges.."[color]"
